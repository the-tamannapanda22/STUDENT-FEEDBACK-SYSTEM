...
import { Plus, Trash2, Tag, Loader2, GitBranch, BookOpen, AlertCircle, Pencil, Upload } from 'lucide-react';
...
const CourseSetup = () => {
  ...
  // Derive unique branches from courses
  ...
  const fetchQuestions = async (courseId) => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`http://localhost:5000/api/admin/questions/${courseId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAddQuestion = async (e) => {
    e.preventDefault();
    if (!newQuestion.trim() || !selectedCourseId) return;
    
    try {
      const token = localStorage.getItem('token');
      const payload = { text: newQuestion, category: selectedCategory, courseId: selectedCourseId };
      ...
      try {
        ...
        for (const row of parsedRows) {
          ...
          if (cols.length >= 2) {
            ...
            questionsToUpload.push({ text: qText, category: mappedCat, courseId: selectedCourseId });
          }
        }
        ...
        const token = localStorage.getItem('token');
        await axios.post('http://localhost:5000/api/admin/questions/bulk', { questions: questionsToUpload, courseId: selectedCourseId }, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSaveEdit = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5000/api/admin/questions/${id}`, { text: editingText }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  if (loading) {
     return <DashboardLayout role="ADMIN" title="Academic Blueprints"><div className="flex justify-center py-20"><Loader2 className="w-8 h-8 text-primary animate-spin" /></div></DashboardLayout>;
  }

  return (
    <DashboardLayout role="ADMIN" title="Course Setup">
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-textMuted mt-1">Design segmented feedback forms mapped to courses.</p>
        </div>
      ...
      {/* Step 1 & 2: Course Selection Box */}
      <div className="card mb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-semibold text-textMuted uppercase tracking-wider mb-2">
              <span className="flex items-center gap-2"><GitBranch className="h-4 w-4" /> Step 1 — Select Branch</span>
            </label>
            <select
          ...
          <div>
            <label className="block text-sm font-semibold text-textMuted uppercase tracking-wider mb-2">
              <span className="flex items-center gap-2"><BookOpen className="h-4 w-4" /> Step 2 — Select Course</span>
            </label>
            <select
      ...
      {selectedCourseId ? (
         ...
         <div className="card p-12 text-center border-dashed border-2 border-surfaceHighlight bg-transparent flex flex-col items-center justify-center text-textMuted">
            <BookOpen className="w-12 h-12 mb-4 opacity-50" />
            <p className="font-medium text-lg text-textBase mb-1">No Course Selected</p>
            <p>Please select a branch and a course to configure its questionnaire.</p>
         </div>
      )}
    </DashboardLayout>
  );
